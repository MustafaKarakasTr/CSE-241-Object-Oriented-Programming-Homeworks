...
public:
    Hex();
    explicit Hex(int _size);  // explicit keyword is used
    Hex(int _size,int _against_cpu);
    //Big Three
    ~Hex();
    Hex(const Hex &rSide);
    const Hex& operator =(const Hex &rSide);
    void playGame();
    ...
    void operator --(); // 2 steps back if against cpu
    void operator --(int); // 1 steps back if against user2
    friend ostream & operator <<(ostream & outputStream,const Hex& h);
    friend istream & operator >>(istream & inputStream,Hex & h);
...
private:
    ...
    void createBoard();
    void createBoard(const Hex& rSide); // for copy constructer

	void makeArraysEmpty(); // make all vectors empty
    ...
    bool **check_table=nullptr;
    int ** value_table_of_AI=nullptr;
